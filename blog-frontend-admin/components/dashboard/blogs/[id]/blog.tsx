...
export async function getBlogById(id: string): Promise<{ data: BlogType }> {
  try {
    ...
    <div className="space-y-3">
      <div className="flex gap-2 flex-col lg:flex-row lg:items-center justify-between">
        ...
        <div className="flex gap-3">
          <UpdateBlog blogData={post.data} />
          <DeleteBlog blogData={post.data} />
        </div>
      </div>
      <div className='space-y-4 md:space-y-6'>
        <div className='w-full'>
          <Image
            ...
            className='rounded-xl w-full h-[204px] md:h-[300px] object-cover'
            width={300}
            height={200}
          />
        </div>
        ...
        {post.data.isPublished ? <p className="text-green-600">Post is published</p> : <p className="text-red-600">Post is not published</p>}
